 ...
 * Summary of changes: Re-raise region name highlight and label above the MIDI note
 * group on height change so the region label is always drawn in front of MIDI notes.
 */
...
MidiRegionView::set_height (double height)
{
	...
	/* _note_group is raised to top in MidiView::init(), putting it above
	   name_highlight and name_text.  Re-raise the name items so the region
	   label is always drawn in front of the MIDI notes. */
	if (name_highlight) {
