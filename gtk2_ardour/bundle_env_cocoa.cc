 ...
 * Summary of changes: Registered custom UI fonts (Manrope, SpaceGrotesk) with both
 * fontconfig and CoreText on macOS, ensuring visibility to the bundled Pango font
 * backend and the dev-tree CoreText backend respectively.
 */
...
void load_custom_fonts()
{
	...
	/* Register the custom fonts with BOTH fontconfig and CoreText.
	 *
	 * The app bundle sets FONTCONFIG_FILE (see above), so the bundled Pango uses
	 * the fontconfig backend — which can only see fonts registered through
	 * fontconfig, not CoreText. The bundle's fonts.conf does not scan
	 * Contents/Resources, so without FcConfigAppFontAddFile() the custom fonts
	 * (Manrope, SpaceGrotesk, …) are invisible and the UI silently falls back to
	 * a system font. This mirrors bundle_env_linux.cc. The CoreText registration
	 * is kept for the dev build, which uses the CoreText backend (FONTCONFIG_FILE
	 * is unset there).
	 */
	FcConfig* config = FcInitLoadConfigAndFonts ();

	auto register_font = [&](const char* filename, const char* label) {
		if (!find_file (ardour_data_search_path(), filename, font_file)) {
			cerr << _("Cannot find ") << label << _(" TrueType font") << endl;
			return;
		}

		/* fontconfig backend — used by the app bundle */
		if (FcFalse == FcConfigAppFontAddFile (config, reinterpret_cast<const FcChar8*> (font_file.c_str()))) {
			cerr << _("Cannot load ") << label << _(" TrueType font.") << endl;
		}

		/* CoreText backend — used by the dev build; best-effort (it returns
		 * false when the font is already registered, which is not an error). */
		CFStringRef ttf = CFStringCreateWithBytes(
				kCFAllocatorDefault, (const UInt8*) font_file.c_str(),
				font_file.length(), kCFStringEncodingUTF8, FALSE);
		CFURLRef fontURL = CFURLCreateWithFileSystemPath(kCFAllocatorDefault, ttf, kCFURLPOSIXPathStyle, TRUE);
		CFErrorRef error;
	...
	register_font("ArdourMono.ttf",   "ArdourMono");
	register_font("ArdourSans.ttf",   "ArdourSans");
	register_font("Manrope.ttf",      "Manrope");
	register_font("SpaceGrotesk.ttf", "SpaceGrotesk");

	/* Make the app-font additions current before gtk_init() builds the font map. */
	if (FcFalse == FcConfigSetCurrent (config)) {
		cerr << _("Failed to activate custom font configuration.") << endl;
	}
#endif
}
